...
import sys
from neural_nlp.benchmarks.weight_extract_utils import CrossRegressedCorrelationWithWeights, \
    linear_regression_with_weights, RegressedCorrelationWithWeights, CartesianProductWeight
...
class _PereiraBenchmark_weights(Benchmark):
    ...
    def _metric(self, source_assembly, target_assembly):
        ...
        if 'append_weights' not in score.attrs:
            score.attrs['appended_weights']=weights
        else:
            score.attrs['appended_weights'].append(weights)
...
class PereiraEncodingWithWeights(_PereiraBenchmark_weights):
    ...
    def __init__(self, **kwargs):
        metric = CrossRegressedCorrelationWithWeights(
            ...
            correlation=pearsonr_correlation(xarray_kwargs=dict(correlation_coord='stimulus_id')),return_weights=True,
            crossvalidation_kwargs=dict(splits=5, kfold=True, split_coord='stimulus_id', stratification_coord=None))
...
def Fedorenko2016EncodingWithWeights(identifier):
    """
    Fedorenko encoding which also return regression weights
    ...
    correlation = pearsonr_correlation(xarray_kwargs=dict(correlation_coord='stimulus_id'))
    # this version also extract the weights
    metric = CrossRegressedCorrelationWithWeights(regression=regression, correlation=correlation,return_weights=True,
                                        crossvalidation_kwargs=dict(splits=5, kfold=True, split_coord='stimulus_id',
                                                                    stratification_coord='sentence_id'))

    return _Fedorenko2016_weights(identifier=identifier, metric=metric)
...
def Fedorenko2016V3EncodingWithWeights(identifier):
    """
    Fedorenko benchmark, language electrodes
    Data 03/24/2020: sentence_electrode_lang_elec_max_window_dat (not demeaned across sentences)
    ...
    data source:
        ...
        https://www.pnas.org/content/113/41/E6256
    """
    benchmark = Fedorenko2016EncodingWithWeights(identifier)
    benchmark._target_assembly = LazyLoad(lambda: load_Fedorenko2016(electrodes='language', version=3))
    return benchmark
